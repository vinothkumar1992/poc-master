...
def face_detection(in_video, out_video):
    args = Namespace(image_size='112,112',
                     model='insightface/models/model-y1-test2/model,0',
                     ga_model='', det=0, flip=0, gpu=0, threshold=1.24)
    ...
    # make path for frames save
    names_path_array = str(out_video)
    lent = len(names_path_array.split("."))
    names_path_array = names_path_array.split(".")
    print(lent)
    name_path = ""
    for i in range(0, lent-1):
        name_path = name_path+str(names_path_array[i])
    print(name_path+"***********************************")

    if not os.path.exists(name_path):
        os.makedirs(name_path)

    video_out = cv2.VideoWriter(out_video, cv2.VideoWriter_fourcc(
        'M', 'J', 'P', 'G'), 24, (save_width, save_height))
    ...
    while True:
        ...
        if frames % 3 == 0:
            ...
            if len(bboxes) != 0:
                ...
                for bboxe in bboxes:
                    ...
                    embedding = embedding_model.get_feature(
                        nimg).reshape(1, -1)
                    ...
                    cv2.putText(
                        frame, text, (bbox[0], y), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
                    cv2.rectangle(
                        frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (255, 0, 0), 2)

                    # cut captured faces and save in directory
                    saved_frames += 1  # multiple of frames

                    if (saved_frames == 10):
                        saved_frames = 0
                        sub_face = frame[bbox[1]:bbox[3], bbox[0]:bbox[2]]
                        FaceFileName = name_path+"/"+text+"_" + \
                            str(frames)+".jpg"  # folder path and random name image
        ...
        # cv2.imshow("Frame", frame)
            fra = cv2.imencode('.jpg', frame)[1].tobytes()
